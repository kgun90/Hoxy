...
class PersonalSettingVC: UIViewController {
    ...
    @objc func submitJoin() {
        ...
        let action = UIAlertAction(title: "확인", style: .default) { action in
            self.joinDataManager.joinProcess(self.joinInfo, currentLocation)
            self.showIndicator()
        }
        presentAlert(title: "회원 가입", message: "가입을 진행하시겠습니까?", isCancelActionIncluded: true,preferredStyle: .alert, with: action)
    }
...
extension PersonalSettingVC: CLLocationManagerDelegate {
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last {
            ...
            currentLocation = GeoPoint(latitude: location.coordinate.latitude, longitude: location.coordinate.longitude)

            viewModel.getUserLocation(location)
        }
    }
